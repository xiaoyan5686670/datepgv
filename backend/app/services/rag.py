...
- SQL 中的「表名」与「列名」（FROM/JOIN/WHERE/GROUP BY 等中引用的物理字段）必须严格依据下方「可用的表结构」；严禁臆造、严禁拼音内联；**引用物理列时**名称必须与「⚠️ 仅允许使用以下列名」行中列出的名称逐字一致（区分大小写），绝不允许自行构造未列出的列名。
- 【面向用户展示】最外层 SELECT 投影的每一列必须使用「表达式 AS 中文别名」，使查询结果列名为简短可读的中文（便于表格与图表展示）。别名可结合字段类型与行内 `-- 注释` 用语自拟，勿再使用裸英文列名作为最终列名；若别名可能与保留字冲突或含特殊字符，按本方言规则用反引号 ` 或双引号包裹别名。
